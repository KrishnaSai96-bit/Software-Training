...
class RecipeObject:
  def __init__(self, ID, Title, Cateogry, Time):
    self.ID = ID
    self.Title = Title
...
#@app.post("/CookBook/CreateRecipeWithoutFastAPI", status_code=status.HTTP_201_CREATED)
async def create_recipe(db:db_dependency):
    #recipeBO = RecipeObject(3,'pongal', 'tiffin', '1 Hr')
    #recipeBO = recipeBO.model_dump()
    idRECIPE = 10
    Recipe_Title = "Pongal"
    Recipe_Category = "Tiffin"
    Recipe_CookinTime = "1 Hr"
    print ("BUNNU" + idRECIPE)
    # RECIPE_Title = recipeBO.Title
    # RECIPE_Category = recipeBO.Category
    # RECIPE_CookingTime = recipeBO.Time
    statement = text("INSERT INTO recipe(idRECIPE, RECIPE_Title, RECIPE_Category, RECIPE_CookingTime) VALUES(:idRECIPE, :RECIPE_Title, :RECIPE_Category, :RECIPE_CookingTime)")
    db.execute(statement)
    db.commit()
    
create_recipe (SessionLocal())
